...
int main(void) {
  ...
  while (!WindowShouldClose()) // Detect window close button or ESC key
  {
    ...
    // Draw body parts
    for (size_t i = 0; i < body_parts; i++) {
      DrawLineEx((i == 0) ? head_position : body_positions[i - 1], body_positions[i], body_radius,
                 LIGHTGRAY);
      DrawCircleV(body_positions[i], body_radius, LIGHTGRAY);
    }
    for (size_t i = 0; i < body_parts; i++) {
      DrawCircleV(body_positions[i], body_radius - 6, WHITE);
    }
